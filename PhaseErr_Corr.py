...
for i , (sub_ref, sub_sam, film_ref, film_sam) in enumerate(zip(*t)):
    ...
    sub_real = (-np.sin(delta_sub - delta_sam))  
    sub_real = sub_real * (R_sub_sam_bd * epsilon_0 *c * (1+n)) / (R_film_sam_bd*d_film)
    
    sub_imag = (1j*np.cos(delta_sub - delta_sam))  
    sub_imag = sub_imag * (R_sub_sam_bd * epsilon_0 *c * (1+n)) / (R_film_sam_bd*d_film)
    


    
    sam_real = (np.sin(delta_sub - delta_sam)) 
    sam_real = sam_real * (R_sub_sam_bd * epsilon_0 *c * (1+n)) / (R_film_sam_bd*d_film)
     
    sam_imag = (-1j*np.cos(delta_sub - delta_sam)) 
    sam_imag = sam_imag  * (R_sub_sam_bd * epsilon_0 *c * (1+n)) / (R_film_sam_bd*d_film)
     

    phase_error_real1 = ((sub_real * 0.1) ** 2)  + ((sam_real*0.1)**2)
    phase_error_real = np.sqrt(phase_error_real1)
    phase_err_real.append(phase_error_real)
    
    phase_error_imag1 = ((sub_imag*0.1)**2) + ((sam_imag*0.1)**2)
