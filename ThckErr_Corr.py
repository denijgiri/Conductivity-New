# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 11:21:10 2024
...
for i , (sub_ref, sub_sam, film_ref, film_sam) in enumerate(zip(*t)):
    ...
    ap_phi2_1 = ((np.cos(delta_sub)*np.cos(delta_sam)) + (np.sin(delta_sub)*np.sin(delta_sam)))* (-R_sub_sam_bd/(R_film_sam_bd*d_film**2))
    ap_phi2_1 = ap_phi2_1 + (1/(d_film)**2)
    ap_phi2_1 = (ap_phi2_1 * epsilon_0 * c) * (1 +n)
    

    ip_phi2_1 = ((-1j*np.cos(delta_sub)*np.sin(delta_sam)) + (1j*np.sin(delta_sub)*np.cos(delta_sam))) * (-R_sub_sam_bd/(R_film_sam_bd*d_film**2))
    ip_phi2_1 = (ip_phi2_1 *  epsilon_0 * c) * (1 +n)

     
    
    phase_error_real = ((ap_phi2_1 ) * 0.00001) ** 2  
    phase_error_real = np.sqrt(phase_error_real)
    phase_err_real.append(phase_error_real)
    
    
    phase_error_imag = ((ip_phi2_1)*0.00001)**2
    phase_error_imag = np.sqrt(phase_error_imag)
