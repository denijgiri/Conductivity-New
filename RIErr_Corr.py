# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 16:47:54 2024
...
for i , (sub_ref, sub_sam, film_ref, film_sam) in enumerate(zip(*t)):
    ...
    freqs_range = (freqs>=0.999*10**12)*(freqs< 1.01*10**12)
    ...
    ap_phi2_1 = (((np.cos(delta_sub)*np.cos(delta_sam)) + (np.sin(delta_sub)*np.sin(delta_sam))) * (R_sub_sam_bd/(R_film_sam_bd*d_film))) - (1/d_film)
    ap_phi2_1 = ap_phi2_1 * epsilon_0 * c
   
    ip_phi2_1 = ((-1j*np.cos(delta_sub)*np.sin(delta_sam)) + (1j*np.sin(delta_sub)*np.cos(delta_sam))) * (R_sub_sam_bd/(R_film_sam_bd*d_film))
    ip_phi2_1 = ip_phi2_1 *  epsilon_0 * c
   
   
    ri_error_real = ((ap_phi2_1* 0.1)) ** 2
    ri_error_real = np.sqrt(ri_error_real)
    rii_err_real.append(ri_error_real)
   
    ri_error_imag = ((ip_phi2_1)*0.1)**2 
    ri_error_imag = np.sqrt(ri_error_imag)
    rii_err_imag.append(ri_error_imag)
    result.append(sigma)
...
rii_err_real = np.array(rii_err_real) 
rii_err_real = np.mean(rii_err_real, axis = 0)
rii_err_real = rii_err_real[freqs_range]
print(rii_err_real)

       
rii_err_imag = np.array(rii_err_imag) 
rii_err_imag = np.mean(rii_err_imag, axis = 0)
rii_err_imag = rii_err_imag[freqs_range]
print(rii_err_imag)
